  ...
  if (error) return (
    <div className="min-h-screen bg-slate-50 p-8 flex flex-col items-center">
       <div className="max-w-md w-full bg-white rounded-2xl shadow-sm border border-red-100 p-8 text-center mt-12">
          ...
          <h2 className="text-xl font-bold text-muted-foreground mb-2">Access Denied</h2>
          <p className="text-muted-foreground text-sm mb-6">{error}</p>
          <button 
            onClick={() => router.back()}
            className="px-6 py-2 bg-card text-white rounded-full text-sm font-medium hover:bg-secondary"
          >
            Go Back
          </button>
       </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3 text-sm text-muted-foreground mt-1">
                ...
                {patient.abha_id && patient.abha_id !== '[REDACTED]' && (
                  <>
                    <span>•</span>
                    <span className="font-mono bg-secondary px-2 py-0.5 rounded text-xs border border-border">
               ...
               <div className="relative border-l-2 border-border ml-4 space-y-8 pb-4">
                  {timeline.map((event: any, idx: number) => (
                    <div key={event.id || idx} className="relative pl-8">
                       <div className="absolute -left-[17px] top-1 bg-white p-1 rounded-full border border-border shadow-sm">
                       ...
                       <div className="bg-white border border-border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
                          <div className="flex justify-between items-start mb-3">
                             <div>
                                <h3 className="font-semibold text-muted-foreground">{getEventTitle(event)}</h3>
                          ...
                          <div className="text-sm text-muted-foreground space-y-2 mt-4">
                             {event._type === 'TRIAGE' && (
                                <div className="bg-slate-50 p-3 rounded-lg border border-border">
                                   <p><strong className="text-muted-foreground">Symptoms:</strong> {event.symptoms?.join(', ')}</p>
                                   <p className="mt-1"><strong className="text-muted-foreground">Recommendation:</strong> {event.recommendation}</p>
                                </div>
                             )}
                             ...
                             {event._type === 'FOLLOW_UP_RECORD' && (
                                <div className="grid grid-cols-2 gap-4 bg-slate-50 p-3 rounded-lg border border-border mt-2">
                                   <div>
