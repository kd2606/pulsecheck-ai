            ...
            <div className={cn("w-full rounded-2xl border p-6 flex items-center gap-4 transition-all duration-500", config.bg, config.border, config.glow)}>
                ...
                <div>
                    ...
                    <p className="text-xs text-white/40 font-medium tracking-wide">
                        {priority === "High Triage Priority" ? "Immediate evaluation recommended." : priority === "Elevated Triage Priority" ? "Monitor parameters closely." : "Baseline parameters stable."}
                    </p>
                </div>
            </div>
        )
                ...
                {[1, 2, 3].map((i) => (
                    <div key={i} className="flex flex-col items-center gap-4 w-1/3 relative group">
                        ...
                        <span className={`text-[10px] font-bold uppercase tracking-[0.2em] transition-colors ${step >= i ? "text-emerald-400" : "text-white/20"}`}>
                            {i === 1 ? "Archive" : i === 2 ? "Protocol" : "Resolve"}
                        </span>
                        {i < 3 && (
                    ...
                    <GlassCard className="!p-10">
                        <div className="space-y-8">
                            <div className="space-y-2">
                                <h2 className="text-3xl font-space font-bold tracking-tight">Sequence <span className="text-white/40">Archive</span></h2>
                                <p className="text-white/40 text-sm font-medium">Capture high-fidelity physiological telemetry for neural analysis.</p>
                            </div>
                            ...
                            <div className={`relative overflow-hidden rounded-[2rem] border border-white/5 bg-black/40 aspect-video flex items-center justify-center ${showCamera ? "block" : "hidden"}`}>
                                ...
                                <Button onClick={capturePhoto} className="absolute bottom-8 bg-white text-black hover:bg-white/90 font-bold px-8 h-12 rounded-xl">
                                    <Camera className="mr-2 h-4 w-4" /> Initialize Capture
                                </Button>
                            </div>
                            ...
                            <div className="flex gap-4">
                                <Button variant="outline" onClick={startCamera} disabled={showCamera} className="h-14 flex-1 rounded-2xl border-white/5 bg-white/[0.02] hover:bg-white/[0.05] font-bold">
                                    <Camera className="mr-2 h-5 w-5 text-emerald-400" /> System Lens
                                </Button>
                                <Button variant="outline" onClick={() => fileInputRef.current?.click()} className="h-14 flex-1 rounded-2xl border-white/5 bg-white/[0.02] hover:bg-white/[0.05] font-bold">
                                    <Upload className="mr-2 h-5 w-5 text-indigo-400" /> External Node
                                </Button>
                            </div>
                            ...
                            {imageData && (
                                <Button onClick={() => setStep(2)} className="w-full h-16 rounded-2xl bg-emerald-500 text-black hover:bg-emerald-400 font-bold text-lg emerald-glow">
                                    Analyze Telemetry <ArrowRight className="ml-2 h-6 w-6" />
                                </Button>
                            )}
                        </div>
                    </GlassCard>
                ...
                <FadeIn delay={0.1} className="max-w-2xl mx-auto w-full">
                    <GlassCard className="!p-10">
                        <div className="space-y-10">
                            <div className="space-y-2">
                                <h2 className="text-3xl font-space font-bold tracking-tight text-white">Diagnostic <span className="text-white/40">Parameters</span></h2>
                                <p className="text-white/40 text-sm font-medium">Input physiological resonance markers.</p>
                            </div>

                            <div className="space-y-8">
                                <div className="space-y-4">
                                    <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-400/80">Itching Intensity</Label>
                                    <Select value={itchingLevel} onValueChange={setItchingLevel}>
                                        <SelectTrigger className="h-14 rounded-2xl border-white/5 bg-white/[0.02] text-white">
                                            <SelectValue placeholder="Resonance level..." />
                                        </SelectTrigger>
                                        <SelectContent className="bg-[#0e0e0e] border-white/5 text-white">
                                            <SelectItem value="none">Zero Interference (None)</SelectItem>
                                            <SelectItem value="mild">Low Resonance (Mild)</SelectItem>
                                            <SelectItem value="severe">Critical Resonance (Severe)</SelectItem>
                                        </SelectContent>
                                    </Select>
                                </div>

                                <div className="space-y-4">
                                    <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-indigo-400/80">Expansion Vector</Label>
                                    <Select value={spreadRate} onValueChange={setSpreadRate}>
                                        <SelectTrigger className="h-14 rounded-2xl border-white/5 bg-white/[0.02] text-white">
                                            <SelectValue placeholder="Vector status..." />
                                        </SelectTrigger>
                                        <SelectContent className="bg-[#0e0e0e] border-white/5 text-white">
                                            <SelectItem value="not_spreading">Static (Not spreading)</SelectItem>
                                            <SelectItem value="slowly">Linear Expansion (Slow)</SelectItem>
                                            <SelectItem value="rapidly">Exponential Growth (Rapid)</SelectItem>
                                        </SelectContent>
                                    </Select>
                                </div>

                                <div className="space-y-4">
                                    <Label className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/40">Mutation Status</Label>
                                    <Select value={recentChanges} onValueChange={setRecentChanges}>
                                        <SelectTrigger className="h-14 rounded-2xl border-white/5 bg-white/[0.02] text-white">
                                            <SelectValue placeholder="Delta status..." />
                                        </SelectTrigger>
                                        <SelectContent className="bg-[#0e0e0e] border-white/5 text-white">
                                            <SelectItem value="no_change">No Delta (Stable)</SelectItem>
                                            <SelectItem value="changed_color_size">Morphological Shift (Size/Color)</SelectItem>
                                            <SelectItem value="bleeding_oozing">Critical Leakage (Bleeding/Oozing)</SelectItem>
                                        </SelectContent>
                                    </Select>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-4 pt-4">
                                ...
                                <Button onClick={handleAnalyze} disabled={!isMetadataComplete} className="h-16 flex-1 rounded-2xl bg-white text-black hover:bg-white/90 font-bold text-lg shadow-xl shadow-white/5">
                                    Initialize Resolve <ArrowRight className="ml-2 h-6 w-6" />
                                </Button>
                            </div>
                        </div>
                    </GlassCard>
                ...
                <FadeIn delay={0.2} className="max-w-4xl mx-auto w-full space-y-8 pb-12">
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-8">
                            <GlassCard className="!p-8 border-emerald-500/20 bg-emerald-500/[0.02]">
                                <div className="space-y-4">
                                    <div className="flex items-center gap-2 text-emerald-400">
                                        ...
                                        <h3 className="text-[10px] font-bold uppercase tracking-[0.2em]">Neural Summary</h3>
                                    </div>
                            ...
                            <div className="space-y-4">
                                <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/40 px-2">Identified Markers</h4>
                        ...
                        <div className="space-y-8">
                            <div className="space-y-4">
                                <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-white/40 px-2">Clinical Protocol</h4>
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                >
                                    <ExternalLink className="h-5 w-5" /> Transmission Report
                                </Button>
                                ...
                                <Button
                                    ...
                                    onClick={async () => {
                                        ...
                                        try {
                                            ...
                                            toast.error("Failed to archive telemetry");
                                        } finally {
                                            setSaving(false);
                                        }
                                    }}
                            ...
                            {results.triagePriority === "High Triage Priority" && (
                                <div className="p-6 rounded-2xl border border-red-500/20 bg-red-500/5 space-y-4 animate-pulse">
                                    <div className="flex items-center gap-2 text-red-400">
                                        ...
                                        <h3 className="text-[10px] font-bold uppercase tracking-[0.2em]">Emergency Node Extraction</h3>
                                    </div>
                    ...
                    <div className="pt-8 border-t border-white/5 text-center">
                        <Button variant="ghost" className="text-white/20 hover:text-white" onClick={() => {
                           ...
                           ← Initialize New Archive Sequence
                        </Button>
                    </div>
                </FadeIn>
            )}
